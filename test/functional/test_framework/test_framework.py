...
TMPDIR_PREFIX = "hodlcash_func_test_"


class HodlCashTestFramework():
    """Base class for a hodlcash test script.

    Individual hodlcash test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use hodlcash-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: hodlcashds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a hodlcashd test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple hodlcashd test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'hodlcashd exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "hodlcashd should have exited with an error"
                else:
                    assert_msg = "hodlcashd should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as hodlcashd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in hodlcash.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in hodlcash.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "hodlcashd"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        if not toPosPhase and not is_powcache_valid:
            ...
            # - Nodes 2 and 3 mint one zerocoin for each denom (tot 6666 HODL) on block 301/302
            ...
            for peer in range(4):
                for j in range(20):
                    ...
                    if nBlocks == 301 or nBlocks == 302:
                        # mints 7 zerocoins, one for each denom (tot 6666 HODL), fee = 0.01 * 8
                        # consumes 27 utxos (tot 6750 HODL), change = 6750 - 6666 - fee
    ...
    ### HodlCash Specific TestFramework ###
    ###################################
    ...
    def test_PoS_chain_balances(self):
        ...
        # - Nodes 2 and 3 have 6666 HODL worth of zerocoins
    ...
    def stake_block(self, node_id,
        ...
        for tx in vtx:
            if not fDoubleSpend:
                # assume txes don't double spend zHODL inputs when fDoubleSpend is false. It needs to
...
class ComparisonTestFramework(HodlCashTestFramework):
    ...
    Sets up some hodlcashd binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "hodlcashd"),
