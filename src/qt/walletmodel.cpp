...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 HODL.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzHODLReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzHODLReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzHODLReset.connect(boost::bind(NotifyzHODLReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzHODLReset.disconnect(boost::bind(NotifyzHODLReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid HodlCash address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid HodlCash address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from HodlCash address");
