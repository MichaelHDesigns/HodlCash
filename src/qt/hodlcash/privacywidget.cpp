...
#include "qt/hodlcash/privacywidget.h"
#include "qt/hodlcash/forms/ui_privacywidget.h"
#include "qt/hodlcash/qtutils.h"
#include "guiutil.h"
#include "qt/hodlcash/denomgenerationdialog.h"
#include "qt/hodlcash/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(HodlCashGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zHODL anonymizes your HODL by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zHODL or convert back to HODL"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 HODL ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zHODL");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zHODL");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zHODL");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zHODL");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zHODL");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zHODL");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zHODL");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zHODL");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zHODL Movements"));
    ...
    // Only Convert to HODL enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzHODL->setTitleClassAndText("btn-title-grey", "Total 0 zHODL");
    ui->btnTotalzHODL->setSubTitleClassAndText("text-subtitle", "Show denominations of zHODL owned.");
    ui->btnTotalzHODL->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select HODL outputs to mint into zHODL.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zHODL");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzHODL, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zHODL");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of HODL to mint into zHODL"));
    }else{
        btnText = tr("Convert back to HODL");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zHODL to convert back into HODL"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzHODL->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzHODL->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zHODL is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zHODL, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zHODL").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zHODL minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zHODL converted back to HODL"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any HODL to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zHODL </b>";
    ...
    ui->btnTotalzHODL->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
