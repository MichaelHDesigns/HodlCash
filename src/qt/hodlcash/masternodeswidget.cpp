...
#include "qt/hodlcash/forms/ui_masternodeswidget.h"
#include "qt/hodlcash/qtutils.h"
#include "qt/hodlcash/mnrow.h"
#include "qt/hodlcash/mninfodialog.h"

#include "qt/hodlcash/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(HodlCashGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i HODL requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 10,000 HODL required."));
            return;
        }
        showHideOp(true);
