...
#include "qt/hodlcash/send.h"
#include "qt/hodlcash/forms/ui_send.h"
#include "qt/hodlcash/addnewcontactdialog.h"
#include "qt/hodlcash/qtutils.h"
#include "qt/hodlcash/sendchangeaddressdialog.h"
#include "qt/hodlcash/optionbutton.h"
#include "qt/hodlcash/sendconfirmdialog.h"
#include "qt/hodlcash/myaddressrow.h"
#include "qt/hodlcash/guitransactionsutils.h"
...
SendWidget::SendWidget(HodlCashGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (HODL)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("HodlCash address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 HODL");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_HODL : AskPassphraseDialog::Context::Send_zHODL, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zHODL is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zHODL selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zHODL transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZHODL_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zHODL require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zHODL transaction
        ...
        emit message("zHODL transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isHODL){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zHODL in your balance to select."));
        }
    }
}
...
void SendWidget::onHODLSelected(bool _isHODL){
    isHODL = _isHODL;
    setCssProperty(coinIcon, _isHODL ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
