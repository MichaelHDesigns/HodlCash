...
CzHODLTracker::CzHODLTracker(std::string strWalletFile)
{
...
CzHODLTracker::~CzHODLTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzHODLTracker::Init()
{
...
bool CzHODLTracker::Archive(CMintMeta& meta)
{
...
bool CzHODLTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzHODLTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzHODLTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzHODLTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzHODLTracker::GetSerialHashes()
{
...
CAmount CzHODLTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzHODLTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzHODLTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzHODLTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzHODLTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzHODLTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzHODLTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzHODLTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzHODLTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzHODLTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzHODLTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzHODLWallet* zHODLWallet)
{
    bool iszHODLWalletInitialized = (NULL != zHODLWallet);
    CMintMeta meta;
    ...
    if (! iszHODLWalletInitialized)
        zHODLWallet = new CzHODLWallet(strWalletFile);
    meta.isSeedCorrect = zHODLWallet->CheckSeed(dMint);
    if (! iszHODLWalletInitialized)
        delete zHODLWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzHODLTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHODLTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzHODLTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzHODLTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzHODLTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzHODLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzHODLWallet* zHODLWallet = new CzHODLWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zHODLWallet);
        }
        delete zHODLWallet;
