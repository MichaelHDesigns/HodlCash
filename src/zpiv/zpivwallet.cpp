...
CzHODLWallet::CzHODLWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHODLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHODLSeed_deprecated()) {
                    LogPrintf("%s: Updated zHODL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzHODLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHODLCount(nCountLastUsed);
    else if (!walletdb.ReadZHODLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHODLWallet::Lock()
{
    seedMaster = 0;
}

void CzHODLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHODLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHODL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHODLWallet::LoadMintPoolFromDB()
{
...
void CzHODLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHODLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHODLWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzHODLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHODL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzHODLWallet::SeedToZHODL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHODLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHODLWallet::GenerateDeterministicZHODL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHODLWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHODLWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzHODLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
