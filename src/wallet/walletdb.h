...
public:
    ...
    bool WriteZHODLSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZHODLSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZHODLSeed_deprecated(uint256& seed);
    bool EraseZHODLSeed();
    bool EraseZHODLSeed_deprecated();

    bool WriteZHODLCount(const uint32_t& nCount);
    bool ReadZHODLCount(uint32_t& nCount);
