...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 HODL at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zHODL send
enum ZerocoinSpendStatus {
    ZHODL_SPEND_OKAY = 0,                            // No error
    ZHODL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZHODL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZHODL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZHODL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZHODL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZHODL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZHODL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZHODL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZHODL_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZHODL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZHODL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZHODL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZHODL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZHODL_SPENT_USED_ZHODL = 14,                      // Coin has already been spend
    ZHODL_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZHODL_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZHODLOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzHODLTracker> zpivTracker;
    ...
    void setZWallet(CzHODLWallet* zwallet);
    CzHODLWallet* getZWallet();
    ...
    /// Get 10000 HODL output and keys which can be used for the Masternode
    ...
    /** zHODL reset */
    boost::signals2::signal<void()> NotifyzHODLReset;
