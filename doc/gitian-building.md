...
*Setup instructions for a gitian build of Hodl Cash using a VM or physical system.*
...
to the HodlCash GitHub release page.
...
- [Building Hodl Cash](#building-hodlcash-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/hodlcash-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/hodlcash-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/hodlcash-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/hodlcash-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/hodlcash-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/hodlcash-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/hodlcash-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@hodlcash.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/hodlcash-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/hodlcash-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/hodlcash-osx-*-build.assert
```
